...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    let i = Ticket::new(title.clone(), description, status.clone());
    match i {
        Ok(n) => n,
        Err(TicketNewError::EmptyTitle) => panic!("Title cannot be empty"),
        Err(TicketNewError::TitleTooLong) => panic!("Title cannot be longer than 50 bytes"),
        Err(TicketNewError::InvalidDescription) => easy_ticket(title, "Description not provided".into(), status)
    }
}
